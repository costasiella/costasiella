...
from ..models import FinancePaymentBatchItem
...
class FinancePaymentBatchItemNode(DjangoObjectType):
    ...
    def get_node(self, info, id):
        ...
        require_login_and_permission(user, 'costasiella.view_financepaymentbatchitem')

        return self._meta.model.objects.get(id=id)


class FinancePaymentBatchItemQuery(graphene.ObjectType):
    finance_payment_batch_items = DjangoFilterConnectionField(FinancePaymentBatchItemNode)
    finance_payment_batch_item = graphene.relay.Node.Field(FinancePaymentBatchItemNode)

    def resolve_finance_payment_batch_items(self, info, **kwargs):
        user = info.context.user
        require_login_and_permission(user, 'costasiella.view_financepaymentbatchitem')
